"""
数据库查询工具模块

该模块提供MySQL数据库连接和查询功能，
专门用于查询menu数据库中的menu_items表的全部信息

"""

import logging

logging.basicConfig(
    level=logging.INFO,
    encoding="utf-8",
)
logger = logging.getLogger(__name__)

import os
from dotenv import load_dotenv
import mysql.connector
from typing import Dict, List, Any

load_dotenv()


class DataBaseConnection:
    """数据库管理"""

    def __init__(self):
        """初始化数据库配置信息"""
        self.host = os.getenv("MYSQL_HOST", "localhost")
        self.port = os.getenv("MYSQL_PORT")
        self.user = os.getenv("MYSQL_USER_NAME")
        self.password = os.getenv("MYSQL_USER_PASSWORD")
        self.db_name = os.getenv("MYSQL_DB_NAME")

        self.connection = None  # 连接对象
        self.cursor = None  # 真正执行sql的对象

    def init_connection(self) -> bool:
        # 初始化数据库连接和游标对象
        try:
            self.connection = mysql.connector.connect(
                host=self.host,
                port=self.port,
                user=self.user,
                password=self.password,
                database=self.db_name,
                charset="utf-8",
            )
            # dictionary=True指定游标返回的结果为字典形式
            self.cursor = self.connection.cursor(dictionary=True)
            logger.info(f"数据库{self.db_name}连接成功")
            return True
        except mysql.connector.Error as e:
            logger.error(f"数据库{self.db_name}连接初始化失败:{e}")
            return False

    def disconnect_connection(self) -> bool:

        try:
            # 1. 关闭游标
            if self.cursor:
                self.cursor.close()
                self.cursor = None
            # 2. 关闭连接
            if self.connection and self.connection.is_connected():
                self.connection.close()
                self.connection = None
            logger.info("关闭数据库资源成功")
            return True
        except mysql.connector.Error as e:
            logger.error(f"关闭数据库资源失败:{e}")
            return False

    def __enter__(self):
        """上下文管理对象入口
        调用时机：实例化后，在with代码块执行之前
        返回值：一定是一个上下文管理对象（这里是self）
        """
        if self.init_connection():
            logger.info("数据库初始化成功")
            return self
        else:
            raise Exception

    def __exit__(self, exc_type, exc, tb):
        """上下文管理对象出口
        调用时机：在with代码块执行之后
        Args:
            exc_type (_type_): 异常类型
            exc (_type_): 异常类型对应的具体说明
            tb (_type_): 记录哪个模块，哪一行代码出现问题，堆栈跟踪
        """
        self.disconnect_connection()

        if exc_type:
            logger.error(f"执行with代码块期间出现异常:{exc}")
        return False


def get_all_menu_items() -> str:
    """
    查询menu_items中的所有菜单信息，并对每一条次啊但信息使用\n连接最终形成一个大字符串（向量化）
    Returns:
        str: str
    """
    try:
        with DataBaseConnection() as db:
            # 1.定义sql语句
            query = """
                SELECT 
                    id, dish_name, price, description, category, 
                    spice_level, flavor, main_ingredients, cooking_method, 
                    is_vegetarian, allergens, is_available
                FROM menu_items 
                WHERE is_available = 1
                ORDER BY category, dish_name
            """
            # 2.执行sql
            db.cursor.execute(query)
            menu_items = db.cursor.fetchall()

            # 3. 处理结果
            if not menu_items:
                logger.info("当前无可用菜品信息")
                return "当前无可用菜品信息"

            menu_items_string = []
            for item in menu_items:
                # 1.格式化辣度级别
                spice_level_mapping = {0: "不辣", 1: "微辣", 2: "中辣", 3: "重辣"}
                format_spice_level = spice_level_mapping.get(
                    item.get("spice_level"), "暂无辣度级别"
                )

                # 2.格式化是否是素食
                format_is_vegetarian = "是" if item.get("is_vegetarian") else "否"

                # 3.格式化菜品描述
                format_description = (
                    item.get("description")
                    if item.get("description", "暂无描述").strip()
                    else "暂无描述"
                )

                # 4.格式化主要食材
                format_main_ingredients = (
                    item.get("main_ingredients")
                    if item.get("main_ingredients", "暂无主要食材").strip()
                    else "暂无主要食材"
                )

                # 5.格式化过敏原
                format_allergens = (
                    item.get("allergens")
                    if item.get("allergens", "暂无过敏原").strip()
                    else "暂无过敏原"
                )

                # 拼接菜品结构为字符串
                menu_string = f"菜品ID:{item['id']}|菜品名称:{item['dish_name']}|价格:¥{item['price']:.2f}|菜品描述:{format_description}|分类:{item['category']}|辣度:{format_spice_level}|口味:{item['flavor']}|主要食材:{format_main_ingredients}|烹饪方法:{item['cooking_method']}|素食:{format_is_vegetarian}|过敏原:{format_allergens}"

                menu_items_string.append(menu_string)
            # 所有菜品字符串用\n连接成一个大字符串
            logger.info(f"查询菜品信息成功,一共查询到{len(menu_items_string)}个菜品")
            return "\n".join(menu_items_string)

    except Exception as e:
        logger.error(f"查询所有菜品信息失败:{e}")
        return "查询所有菜品信息失败"


def get_menu_items() -> List[Dict[str, Any]]:
    """前端菜品区域展示

    Returns:
        List[Dict[str,Any]]: 字典列表，菜品信息列表
    """
    try:
        with DataBaseConnection() as db:
            query = """
                    SELECT 
                        id, dish_name, price, description, category, 
                        spice_level, flavor, main_ingredients, cooking_method, 
                        is_vegetarian, allergens, is_available
                    FROM menu_items 
                    WHERE is_available = 1
                    ORDER BY category, dish_name
                    """
            
            db.cursor.execute(query)

            menu_items = db.cursor.fetchall()
            if not menu_items:
                logger.error("暂无菜单列表")
                return []
            
            menu_items_list=[]
            for item in menu_items:
                # 辣度等级转换
                spice_levels = {0: "不辣", 1: "微辣", 2: "中辣", 3: "重辣"}
                spice_text = spice_levels.get(item['spice_level'], "未知")

                # 处理数据
                processed_item = {
                    "id": item['id'],
                    "dish_name": item['dish_name'],
                    "price": float(item['price']),
                    "formatted_price": f"¥{item['price']:.2f}",
                    "description": item['description'] or "暂无描述",
                    "category": item['category'],
                    "spice_level": item['spice_level'],
                    "spice_text": spice_text,
                    "flavor": item['flavor'] or "暂无口味",
                    "main_ingredients": item['main_ingredients'] or "暂无主要食材",
                    "cooking_method": item['cooking_method'] or "暂无烹饪方法",
                    "is_vegetarian": bool(item['is_vegetarian']),
                    "vegetarian_text": "是" if item['is_vegetarian'] else "否",
                    "allergens": item['allergens'] if item['allergens'] and item['allergens'].strip() else "暂无过敏原",
                    "is_available": bool(item['is_available'])
                }

                menu_items_list.append(processed_item)
                
            logger.info(f"获取菜单列表成功，一共{len(menu_items_list)}个菜品")
            return menu_items_list
            
    except Exception as e:
        logger.error(f"获取菜品列表失败{e}")
        return []


# if __name__ == "__main__":
#     # menu = get_all_menu_items()
#     # print(menu)

#     menu_list=get_menu_items()
#     for item in menu_list:
#         print(item)
